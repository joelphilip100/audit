...
def get_all_employees(db: Session) -> list[employee_models.Employee]:
    return repository.get_all_employees(db)

def update_employee(gpn: str, updated_data: employee_schema.EmployeeUpdateRequest, db: Session) -> employee_models.Employee:
...
def delete_employee(gpn: str, db: Session) -> employee_models.Employee:
    employee = get_employee_by_gpn(gpn, db)
    return repository.delete_employee(employee, db)
